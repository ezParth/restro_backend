...
func GetMenu() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error occured while fetching a particular menu from database"})
			return
		}
...
func CreateMenu() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, insertionErr := menuCollection.InsertOne(ctx, menu)
		defer cancel()
		if insertionErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Menu Item was not created, " + insertionErr.Error()})
			return
		}
